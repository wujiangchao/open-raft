...
/**
 * Status封装操作的结果。它可能意味着成功，
 * 或者，它可以用相关的错误消息指示错误。它是合适的
 * 用于传递比error_code信息更丰富的函数状态
 * exception-forbidden代码。这个实用程序的灵感来自于leveldb::Status。
 * 多个线程可以调用一个Status上的const方法外部同步，但如果任何线程可能调用
 * 非const方法，所有访问相同Status的线程都必须使用外部同步。
 * 由于失败状态需要分配内存，所以应该小心失败状态频繁。
 */
