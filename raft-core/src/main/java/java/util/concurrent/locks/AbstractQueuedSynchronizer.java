...
public abstract class AbstractQueuedSynchronizer
    ...
    static final class Node {
         ...
         * 节点在等待队列中，节点线程等待在Condition上，当其他线程对Condition调用了signal()方法后，
    ...
    private void unparkSuccessor(Node node) {
         ...
         * 为啥从tail开始遍历
         * 在enq方法中 在 cas 操作之后，t.next=node 操作之前。存在其他线程调用 unlock 方法从 head开始往后遍历，
         * 由于 t.next=node 还没执行意味着链表的关系还没有建立完整。就会导致遍历到 t 节点的时候被中断。所以从后往前遍历，一定不会存在这个问题。
         */
        Node s = node.next;
     ...
     * 使用CAS将节点状态由INITIAL设置成SIGNAL，表示当前线程阻塞。当compareAndSetWaitStatus
     * 设置失败则说明shouldParkAfterFailedAcquire方法返回false，然后会在acquireQueued()
     * 方法中for (;;)死循环中会继续重试，直至compareAndSetWaitStatus设置节点状态位为SIGNAL
     * 时shouldParkAfterFailedAcquire返回true时才会执行方法
     *
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        ...
        if (ws > 0) {
             ...
             * 如果节点的状态大于0，说明节点被取消了，
             * 那就一直往前找到没有取消的节点
             * 当前节点的 ws > 0, 则为 Node.CANCELLED 说明前驱节点已经取消了等待锁(由于超时或者中断等原因),既然前驱节点不等了,
             * 那就继续往前找, 直到找到一个还在等待锁的节点,然后我们跨过这些不等待锁的节点, 直接排在等待锁的节点的后面 (是不是很开心!!!)
             */
            do {
                node.prev = pred = pred.prev;
            } while (pred.waitStatus > 0);
    ...
    private final boolean parkAndCheckInterrupt() {
        ...
        //如果线程是因为响应中断了才被唤醒，Thread.interrupted() 会将中断位clear
        //所以返回为ture 需要selfInterrupt补偿一下(线程在调用 acquireQueued 方法的时候是不会响应中断请求的)
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (; ; ) {
                ...
                //没有走上面的if分支说明获取锁失败，线程进入等待状态等待获取独占式锁
    ...
    public final boolean release(int arg) {
        if (tryRelease(arg)) {
            Node h = head;
            //shouldParkAfterFailedAcquire中head.waitStatus被赋值-1
